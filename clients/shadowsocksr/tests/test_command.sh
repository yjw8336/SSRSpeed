...
assert "$LOCAL 2>&1 -m rc4-md5 -k testrc4 -s 192.168.2.191 -p 8388 -d start | grep WARNING |  awk -F\"WARNING\" '{print \$2}'" "  warning: server set to listen on 192.168.2.191:8388, are you sure?"
$LOCAL 2>/dev/null 1>/dev/null -m rc4-md5 -k testrc4 -s 192.168.2.191 -p 8388 -d stop
...
assert "$SERVER 2>&1 --forbidden-ip 192.168.2.191/4a -m rc4-md5 -k 12345 -p 8388 -s 0.0.0.0 -d start | grep ERROR |  awk -F\"ERROR\" '{print \$2}'" ": Not a valid CIDR notation: 192.168.2.191/4a"
$LOCAL 2>/dev/null 1>/dev/null -m rc4-md5 -k testrc4 -s 192.168.2.191 -p 8388 -d stop
